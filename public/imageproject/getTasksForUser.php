...
$result = $db->query("SELECT * FROM `tasks` WHERE `is_seen` = 0 AND `user_id` = " . $db->escape($_POST['user_id']) );
...
if($db->num_rows($result))
{
	...
	$response["status"] = 1;
	$response["message"] = "There is result";
}
else
{
	$response["status"] = 0;
	$response["message"] = "Error occured during taks extraction!";
}
