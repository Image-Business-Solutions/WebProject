...
$result = $db->query("SELECT `categories`.`id`, `categories`.`name` FROM `categories`, `rel_companies_categories`
					WHERE `categories`.`id` = `rel_companies_categories`.`category_id`
					AND `rel_companies_categories`.`company_id` = " . $db->escape($_POST['company_id']) ."
